...
function worldSvg(map: MapDef, era: EraDef, flags: string[], nodes: { n: MapNode; ok: boolean }[]): string {
  const ridge = (y: number, amp: number) => `M0 ${y} ${Array.from({ length: 9 }, (_, i) => `L${i * 200} ${y - ((i * 37) % 3) * amp - (i % 2) * amp}`).join(' ')} L1600 ${y} L1600 0 L0 0 Z`;
  const quiet = era.id === '2148';
  return `<svg class="world" viewBox="0 0 ${map.width} ${map.height}" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg">
    <rect width="${map.width}" height="${map.height}" fill="${era.palette.bg}"/>
    ${quiet ? '' : `<g stroke="${era.palette.accent2}" stroke-width="0.5" opacity="0.5">${Array.from({ length: 18 }, (_, i) => `<line x1="0" y1="${i * 50}" x2="1600" y2="${i * 50}"/>`).join('')}${Array.from({ length: 32 }, (_, i) => `<line x1="${i * 50}" y1="0" x2="${i * 50}" y2="900"/>`).join('')}</g>`}
    <path d="${ridge(150, 40)}" fill="${era.palette.surface}" stroke="${era.palette.ink}" stroke-width="${era.lineWeight}"/>
    <path d="M0 900 L0 780 Q300 700 600 820 T1200 780 T1600 840 L1600 900 Z" fill="${era.palette.surface}" stroke="${era.palette.ink}" stroke-width="${era.lineWeight}"/>
    <path d="M1180 0 q-60 260 40 470 t-30 430" fill="none" stroke="${quiet ? '#3B6D11' : era.palette.accent}" stroke-width="${quiet ? 10 : 3}" opacity="${quiet ? 0.5 : 0.7}"/>
    ${quiet ? `<g fill="#3B6D11" opacity="0.35">${Array.from({ length: 24 }, (_, i) => `<ellipse cx="${(i * 233) % 1600}" cy="${300 + (i * 131) % 500}" rx="${30 + (i % 4) * 12}" ry="${10 + (i % 3) * 4}"/>`).join('')}</g>` : ''}
...
export function mapScreen(root: HTMLElement, ctx: Ctx, state: GameState): ScreenHandle {
  ...
  const updateNear = () => {
    ...
    ctx.setPrompts(prompts(
      ...
      { btn: 'y', label: 'Tech trees' }, { btn: 'x', label: 'Party' }, { btn: 'start', label: 'Save' }, { btn: 'select', label: 'Settings' },
    ));
  };
  const enter = () => {
    if (!nearNode) return;
  ...
  return {
    input(btn) {
      ...
      else if (btn === 'x') { store.dispatch({ type: 'SET_MAP_POS', x, y }); store.dispatch({ type: 'SET_SCREEN', screen: { id: 'inventory' } }); }
