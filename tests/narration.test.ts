...
function withLog(b: BattleState, texts: string[]): BattleState {
  const log: BattleLogEntry[] = texts.map((text) => ({ turn: 1, text, kind: 'hit' }));
  return { ...b, log: [...b.log, ...log] };
}
...
describe('battle narration', () => {
  ...
  it('pages long enemy turns three lines at a time', () => {
    const b = { ...battle(), log: [] };
    syncNarration(b);
    syncNarration(withLog(b, ['one', 'two', 'three', 'four', 'five']));
  ...
  it('only reports a page as new when the box was empty, so sound plays once', () => {
    const b = { ...battle(), log: [] };
    syncNarration(b);
    expect(syncNarration(withLog(b, ['one']))).not.toBeNull();
    const more = { ...withLog(b, ['one']), log: withLog(b, ['one', 'two']).log };
    expect(syncNarration(more)).toBeNull();
    expect(narrationPending()).toBe(true);
  });

  it('starts over for a new battle and drops queued lines a Rewind erased', () => {
    const first = withLog({ ...battle(1), log: [] }, ['a', 'b']);
    syncNarration(first);
    expect(narrationPending()).toBe(true);
    syncNarration({ ...battle(2, 'kell_2148_walls'), log: [] });
    expect(narrationPending()).toBe(false);
    const long = withLog({ ...battle(2, 'kell_2148_walls'), log: [] }, ['x', 'y']);
    syncNarration(long);
    advanceNarration();
    syncNarration({ ...long, log: [] });
    expect(narrationPending()).toBe(false);
  });
