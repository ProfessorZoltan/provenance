...
export function librarySvg(id: string, role: string, viewBox = '0 0 32 48', className = 'rig', pixelated = true): string | undefined {
  const url = artAssetUrl(id, role);
  if (!url) return undefined;
  const [, , w, h] = viewBox.split(' ');
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${viewBox}" class="${className}" shape-rendering="crispEdges" aria-label="${id.replaceAll('_', ' ')}"><image href="${url}" width="${w}" height="${h}" preserveAspectRatio="xMidYMid meet" style="image-rendering:${pixelated ? 'pixelated' : 'auto'}"/></svg>`;
}
