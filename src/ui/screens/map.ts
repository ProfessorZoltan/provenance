...
const ICONS: Record<string, (era: EraDef) => string> = {
  ...
  capitol: (era) => `<polygon points="-48,-6 0,-34 48,-6" fill="${era.palette.surface}" stroke="${era.palette.ink}" stroke-width="${era.lineWeight}"/>${[-34, -17, 0, 17, 34].map((x) => `<rect x="${x - 4}" y="-6" width="8" height="26" fill="${era.palette.surface}" stroke="${era.palette.ink}" stroke-width="${era.lineWeight}"/>`).join('')}<rect x="-50" y="20" width="100" height="8" fill="${era.palette.surface}" stroke="${era.palette.ink}" stroke-width="${era.lineWeight}"/><rect x="-6" y="-26" width="12" height="10" fill="${era.palette.accent}" opacity="0.85"/>`,
  ruin: (era) => `${[-34, -6, 26].map((x, i) => `<rect x="${x}" y="${-8 - i % 2 * 14}" width="9" height="${28 + (i % 2) * 14}" fill="${era.palette.surface}" stroke="${era.palette.ink}" stroke-width="${era.lineWeight}"/>`).join('')}<rect x="6" y="14" width="34" height="8" fill="${era.palette.surface}" stroke="${era.palette.ink}" stroke-width="${era.lineWeight}" transform="rotate(-14 23 18)"/><rect x="-50" y="20" width="100" height="8" fill="${era.palette.surface}" stroke="${era.palette.ink}" stroke-width="${era.lineWeight}"/>`,
  campus: (era) => `${[-44, -6, 26].map((x, i) => `<rect x="${x}" y="${-10 - (i === 1 ? 12 : 0)}" width="${i === 1 ? 26 : 24}" height="${30 + (i === 1 ? 12 : 0)}" fill="${era.palette.surface}" stroke="${era.palette.ink}" stroke-width="${era.lineWeight}"/>`).join('')}<rect x="-50" y="20" width="100" height="6" fill="${era.palette.surface}" stroke="${era.palette.ink}" stroke-width="${era.lineWeight}"/><circle cx="7" cy="-16" r="4" fill="${era.palette.accent}"/>`,
  vault: (era) => `<rect x="-38" y="-24" width="76" height="46" fill="${era.palette.surface}" stroke="${era.palette.ink}" stroke-width="${era.lineWeight + 1}"/><circle cx="0" cy="-1" r="14" fill="none" stroke="${era.palette.ink}" stroke-width="${era.lineWeight + 1}"/><circle cx="0" cy="-1" r="4" fill="${era.palette.accent}"/>${[-1, 1].map((d) => `<line x1="${d * 9}" y1="-10" x2="${d * 9}" y2="8" stroke="${era.palette.ink}" stroke-width="${era.lineWeight}"/>`).join('')}`,
  core: (era) => `<polygon points="0,-36 30,-14 30,14 0,34 -30,14 -30,-14" fill="${era.palette.surface}" stroke="${era.palette.ink}" stroke-width="${era.lineWeight}"/>${[0, 1, 2].map((i) => `<circle class="amb-pulse" cx="0" cy="0" r="${8 + i * 8}" fill="none" stroke="${era.palette.accent2}" stroke-width="2" opacity="${0.8 - i * 0.22}"/>`).join('')}<circle r="4" fill="${era.palette.accent}"/>`,
